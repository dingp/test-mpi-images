FROM docker.io/nvidia/cuda:13.2.0-cudnn-devel-ubuntu24.04
...
ARG mpi4py=4.1.0
ENV LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/usr/local/cuda-13.2/compat/:/usr/local/cuda-13.2/targets/x86_64-linux/lib/stubs
RUN MPICC="mpicc -shared" pip install --force-reinstall --no-cache-dir --break-system-packages --no-binary=mpi4py mpi4py==$mpi4py
